    ...
    while IFS= read -r TARGET_IP; do
        ...
        if echo "$SCRIPT_INFO" | grep -q "printer-info"; then
            ...
            if echo "$SCRIPT_INFO" | grep -qi "$BLACKLIST_BRANDS"; then
                ...
                # ===== ФИНАЛЬНАЯ PJL-ВЕРИФИКАЦИЯ (ПОВЫШЕНИЕ ДОСТОВЕРНОСТИ) =====
        # Проверяем, открыт ли порт 9100 и устройство все еще подозрительное
        if [ "$SUSPICIOUS" = true ] && echo "$SERVICE_INFO" | grep -q "9100/open"; then
            echo "      🔬 Проверяем подлинность через PJL..."
            log_suspicious "PJL_VERIFICATION_START" "$TARGET_IP" "Начало PJL-верификации для подозрительного устройства"
            
            # Пытаемся получить ID устройства через PJL
            PJL_RESPONSE=$(timeout 5 echo "@PJL INFO ID" | nc -w 3 "$TARGET_IP" 9100 2>/dev/null | tr -d '\0' | head -1 || true)
            
            # Логируем сам факт проведения проверки и полученный ответ
            log_suspicious "PJL_VERIFICATION_ATTEMPT" "$TARGET_IP" "Отправлена команда: @PJL INFO ID"
            
            # Если ответ не пустой и содержит ключевые слова принтера - понижаем уровень угрозы
            if [[ -n "$PJL_RESPONSE" ]]; then
                ...
                if echo "$PJL_RESPONSE" | grep -qi "PJL\|@PJL\|HP\|Canon\|Xerox\|Epson\|Brother"; then
                    echo "      ✅ PJL Check: Устройство ответило как настоящий принтер. Понижаем уровень угрозы."
                    log_suspicious "PJL_VERIFICATION_PASSED" "$TARGET_IP" "Устройство ответило как принтер: $PJL_RESPONSE"
                    SUSPICIOUS=false
                    REASONS=("Обнаружены признаки принтера (PJL ответ: $PJL_RESPONSE)")  # Перезаписываем причины
                else
                    echo "      ⚠️ PJL Check: Получен нестандартный ответ. Подтверждаем угрозу."
                ...
                echo "      ❗ PJL Check: Устройство не ответило на PJL-запрос. Подтверждаем угрозу."
                log_suspicious "PJL_VERIFICATION_TIMEOUT" "$TARGET_IP" "Устройство не ответило на PJL INFO ID"
                REASONS+=("Не ответил на PJL INFO ID (таймаут или отсутствие ответа)")
            fi
        fi
        ...
        if [ "$SUSPICIOUS" = true ]; then
            ...
            echo "      ✅ Устройство в порядке (настоящий принтер или ложное срабатывание)"
            log_suspicious "DEVICE_VERIFIED" "$TARGET_IP" "Устройство верифицировано как принтер через PJL"
        fi
